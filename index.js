...
.ni-world-toggle:hover{opacity:1!important}
    ...
    // 底栏导航（移动端触屏：click 未合成时由 touchend 兜底触发）
    function niNavSwitch(page, btn) {
    ...
    let _niNavTouchStart = null;
    let _niNavPending = null;
    $app.on('touchstart', '.ni-nav-btn', function (e) {
        const t = e.touches?.[0] || e.changedTouches?.[0];
        _niNavTouchStart = t ? { x: t.clientX, y: t.clientY, btn: this } : null;
    });
    $app.on('touchend', '.ni-nav-btn', function (e) {
        const t = e.changedTouches?.[0];
        const start = _niNavTouchStart;
        _niNavTouchStart = null;
        if (!t || !start || start.btn !== this) return;
        if (Math.hypot(t.clientX - start.x, t.clientY - start.y) > 12) return; // 滑动不算点击
        const page = this?.dataset?.page;
        if (!page) return;
        _niNavPending = { btn: this, page, ts: Date.now() };
        setTimeout(() => {
            if (_niNavPending && Date.now() - _niNavPending.ts >= 450) {
                const p = _niNavPending;
                _niNavPending = null;
                if (p.btn?.isConnected) niNavSwitch(p.page, p.btn);
            }
        }, 450);
    });
    $app.on('click', '.ni-nav-btn', function () {
        _niNavPending = null;
        const page = this?.dataset?.page || $(this).data('page');
        if (page) niNavSwitch(page, this);
    });
