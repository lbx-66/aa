...
export const PENETRATION_TERMS = [
    '插入', '进入', '抽送', '抽插', '顶送', '冲刺', '贯入', '交合', '交媾',
    '肉棒', '阴茎', '挺进', '送入', '没入', '撞入', '深入体内'
];

export const PACE_TERMS = ['加速', '节奏', '越来越快', '猛烈', '缓慢', '撞击', '起伏', '挺动', '晃腰', '律动'];

export const PEAK_TERMS = ['高潮', '痉挛', '射精', '喷出', '顶点', '泄出', '释放', '战栗着绷紧'];

export const ENDING_TERMS = [
    '退出', '退出来', '抽身', '停下', '相拥', '抱在怀里', '睡去', '平复',
...
export function buildScenesText(scenes, { withAnchor = true } = {}) {
    return (Array.isArray(scenes) ? scenes : []).map((w, i) => {
        ...
        return `${seg}${stagePart ? `｜${stagePart}` : ''}${modePart}${safePart}${anchorPart}`;
    }).join('\n');
}
...
export function analyzeChapter(text, { chapterNumber = 0, bookProfile = 'normal', config = {}, autoNames = [] } = {}) {
    ...
    const windows = clusters.map((cluster, wi) => {
        ...
        const risk = safetyReason(cluster.scope, chapterNumber, names, config);
