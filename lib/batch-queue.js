...
export function createBatchQueueController({
    ...
    function collectQueue() {
        ...
        items.forEach((item, index) => {
            let ok = false;
            try { ok = isEligible(item); } catch (err) {
                logger.warn?.('[BatchQueue] isEligible 异常，按不可处理跳过:', err?.message || err);
            }
            if (ok) queue.push({ item, index });
        });
        return queue;
    }
    ...
    async function _run() {
        ...
        await Promise.all(Array.from({ length: workers }, async () => {
            while (!state.stop) {
                ...
                let eligible = true;
                try { eligible = isEligible(item); } catch (err) {
                    logger.warn?.('[BatchQueue] isEligible 异常，按不可处理跳过:', err?.message || err);
                    eligible = false;
                }
                if (!eligible) { completed++; continue; } // 运行中被外部改状态
                const controller = new AbortControllerClass();
                state.controllers.set(key, controller);
                // 状态前置/进度回调抛错不能挂死 worker（否则 Promise.all 永不结束、
                // state.running 永远 true，暂停/取消全部失效）
                try { setProcessingStatus(item, index); } catch (err) {
                    logger.warn?.('[BatchQueue] setProcessingStatus 异常:', err?.message || err);
                }
                try {
                    emit({
                        current: item,
                    ...
                    logger.warn?.('[BatchQueue] onProgress 异常:', err?.message || err);
                }
                ...
                try {
                    ...
                    if (wasSkipped) {
                        try { setSkippedStatus(item, index); } catch (e) { logger.warn?.('[BatchQueue] setSkippedStatus 异常:', e?.message || e); }
                    } else if (state.stop) {
                        try { resetStatus(item, index); } catch (e) { logger.warn?.('[BatchQueue] resetStatus 异常:', e?.message || e); }
                    } else {
                        logger.warn?.('[BatchQueue] 处理失败:', err?.message || err);
                        try { setFailedStatus(item, index, err); } catch (e) { logger.warn?.('[BatchQueue] setFailedStatus 异常:', e?.message || e); }
                    }
                } finally {
                    state.controllers.delete(key);
                    // 只有正常完成/明确失败才算"已完成"；被暂停复位/跳过的项不虚增进度
                    if (finishedNormally || !state.stop) completed++;
                }
            }
        }));
