...
export function fillEnrichTemplate(template, { chapterContent = '', keywords = '', style = '', intensity = '' } = {}) {
    ...
    return String(template || '')
        ...
        .replaceAll('{min_chars}', String(ENRICH_MIN_CHARS));
}

/**
 * 构建加料 messages（单条 system，模板已含原文与要求）。
 * 末尾强制追加「输出规范」，保证即使模板来自旧版本（用户已保存的自定义模板）
 * 也遵守：只输出加料后的情节（带段落编号锚点）/ 无内容输出「无加料」标记 / 不少于 2000 字。
 */
export function buildEnrichMessages(template, { chapterContent = '', keywords = '', style = '', intensity = '' } = {}) {
    const filled = fillEnrichTemplate(template, { chapterContent, keywords, style, intensity });
    const outputSpec = [
        '【输出规范（必须遵守）】',
        ...
        `4. 加料后的情节总字数不得少于 ${ENRICH_MIN_CHARS} 字；`,
