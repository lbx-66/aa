...
import { sceneSubtypeLabel, sceneTypeLabel } from './judge-system.js';
 ...
 * contextNotes：可选「本章情报」块（人物卡/前史卡，来自原著分析），置于消息最前，
 * 让模型先建立人物关系认知再读原文；空串时完全不影响消息结构。
 */
export function buildEnrichMessages(template, { chapterContent = '', keywords = '', style = '', intensity = '', minChars = ENRICH_MIN_CHARS, contextNotes = '' } = {}) {
    ...
    const outputSpec = [
        ...
        `6. 本任务是批量章节加料的后台写作任务，不是对话：不要打招呼、开场白、角色自述或结尾总结；若上方任何设定与本规范冲突（如字数要求、对话式回复、整章重写、思考过程输出），一律以本规范为准。`,
    ].join('\n');
    const notes = String(contextNotes || '').trim();
    const head = notes ? `【本章情报】\n${notes}\n\n` : '';
    return [{ role: 'system', content: `${head}${filled}\n\n${outputSpec}` }];
}
...
// 场景子类型 → 加料方向注意（本地引擎无 AI 建议时的类型化兜底；宁缺毋滥，只约束方向不约束内容）
const SCENE_SUBTYPE_DIRECTIONS = {
    solo: '本章场景为女性角色单人自慰/自我抚慰：加料须保持单人情节与女性视角，可展开独处氛围、动作细节与身心感受，不得凭空引入第二人参与性行为。',
    mutual: '本章场景为互慰/手交等未插入的边缘行为：加料应保持「未插入」的边缘推进与克制分寸，除非原文明确发展至插入，否则不要擅自转为性交。',
    oral: '本章场景涉及口交：可按口交的方向推进展开，是否转入其他行为以原文为准。',
};

/**
 * 从判定结果生成加料参考摘要（注入模板 {keywords}）：
 * 场景类型/子类型 + 承接窗口注意 + 子类型加料方向 + AI 加料建议（enrich_hints）
 * + 跨章补全说明（promoNote）→ 高价值块前置、永不被截断；
 * 判定证据（evidence，可能很长）置于最后，按总预算 2000 字符裁剪。
 */
export function buildEnrichKeywordsSummary(judge) {
    ...
    // 场景类型与子类型（explicit_sex 之外的类型理论上不会走到加料，展示出来防越界）
    if (judge.sceneType) {
        const label = sceneTypeLabel(judge.sceneType);
        const sub = judge.sceneSubtype && judge.sceneSubtype !== 'unclear' ? sceneSubtypeLabel(judge.sceneSubtype) : '';
        head.push(`场景类型：${label || judge.sceneType}${sub ? `（${sub}）` : ''}`);
    }

    // 承接窗口注意：本章含有上一章性爱场景的承接描写时，禁止回忆/闪回式补写
    if (judge.carryover?.suppressed && judge.carryover.carryoverOnly === false) {
        head.push('注意：本章部分窗口为上一章性爱场景的承接/次日描写，加料只针对本章自有场景，不要以回忆或闪回方式重写上一章已发生的性爱。');
    }

    // 子类型加料方向（本地窗口/无 AI 建议时的兜底；solo/mutual 必须方向约束）
    const direction = judge.sceneSubtype ? SCENE_SUBTYPE_DIRECTIONS[judge.sceneSubtype] : '';
    if (direction) head.push(direction);

    // AI 判定输出的加料建议（仅 AI/批量路径存在）
    if (Array.isArray(judge.enrichHints) && judge.enrichHints.length) {
        head.push(`加料建议（判定时给出）：\n${judge.enrichHints.map((h, i) => `${i + 1}. ${h}`).join('\n')}`);
    }

    // 两章间省略被归位到本章时写入的章末补全说明（reconcile 提升）
    if (judge.promoNote) head.push(`跨章补全说明：${judge.promoNote}`);

    // 判定证据放最后，按剩余预算裁剪（高价值块优先保留）
    const resultText = judge.result === 'yes' ? '是' : judge.result === 'doubt' ? '存疑' : '否';
    let evidence = '';
    if (judge.mode === 'keyword' || judge.mode === 'keyword-hybrid') {
        evidence = judge.evidence || '';
    } else if (judge.evidence) {
        evidence = `判定结果：${resultText}。依据：${judge.evidence}`;
    } else if (judge.mode === 'ai' || judge.mode === 'batch') {
        evidence = `判定结果：${resultText}`;
    }
    ...
    const text = (() => {
        if (!evidence) return headText;
        if (!headText) return evidence;
        // 头部高价值块优先，剩余预算给证据
        const remain = SUMMARY_BUDGET - headLen;
        if (remain < 120) return headText;
        return `${headText}\n${evidence.slice(0, Math.max(80, remain))}`;
    })();
    if (!text) return '（无）';
    return text.length > SUMMARY_BUDGET ? `${text.slice(0, SUMMARY_BUDGET)}…` : text;
}

/**
 * 构建加料「本章情报」块（纯函数，可单测）：
 * 把清洗管道产出的角色表与剧情节点，按文本匹配注入加料请求，解决
 * 人称混淆/称谓错/OOC（人物卡）与关系进展错位/与后续剧情冲突（前史卡）。
 * 无匹配数据时返回空串（调用方静默跳过，不构成硬依赖）。
 * @param {string} text 章节正文
 * @param {object} opts
 *   - characters: [{name, role, personality, relations, aliases:[{text}|string]}] 角色表
 *   - plotNodes: [{title, body}] 主线剧情节点（按剧情时间顺序）
 *   - maxCharacters: 人物卡数量上限（默认 5）
 *   - maxHistory: 前史节点条数上限（默认 2，取最早提及相关角色的节点）
 *   - maxTraitChars: 性格/关系/节点正文单条截断（默认 60）
 */
export function buildEnrichContextNotes(text, {
    characters = [],
    plotNodes = [],
    maxCharacters = 5,
    maxHistory = 2,
    ...
    const cut = (s, limit) => {
        ...
        const max = cap(limit || maxTraitChars);
        return str.length > max ? `${str.slice(0, max)}…` : str;
    };
    const charNames = c => {
        const out = [String(c?.name || '').trim()];
        (Array.isArray(c?.aliases) ? c.aliases : []).forEach(a => {
            const v = String(typeof a === 'string' ? a : a?.text || '').trim();
            if (v && v.length >= 2) out.push(v);
        });
        return [...new Set(out)].filter(Boolean);
    };

    // A1 人物卡：名字/别名出现在本章文本的角色，按出现次数取前 N
    const related = [];
    for (const c of Array.isArray(characters) ? characters : []) {
        if (!c || !String(c?.name || '').trim()) continue;
        const names = charNames(c);
        let count = 0;
        for (const n of names) {
            let from = 0;
            while ((from = source.indexOf(n, from)) !== -1) { count++; from += n.length; }
        }
        if (count > 0) related.push({ c, names, count });
    }
    related.sort((a, b) => b.count - a.count || String(a.c.name).localeCompare(String(b.c.name)));

    const lines = [];
    related.slice(0, cap(maxCharacters)).forEach(({ c, names }) => {
        const role = String(c?.role || '').trim();
        const aliasText = names.filter(n => n !== c.name).slice(0, 4).join('、');
        const traits = [String(c?.personality || '').trim(), String(c?.relations || '').trim()]
            .filter(Boolean).join('；');
        const seg = `· ${c.name}${role ? `（${role}）` : ''}${aliasText ? `（别名：${aliasText}）` : ''}`;
        lines.push(traits ? `${seg}：${cut(traits)}` : seg);
    });

    // A3 前史卡：主线节点中提及相关角色的最早几条（main 按剧情时间顺序，
    // 取最早 = 相识与关系建立期，避免"未来剧情"剧透误导加料）
    const nodes = Array.isArray(plotNodes) ? plotNodes : [];
    if (lines.length && nodes.length) {
        const allNames = related.flatMap(r => r.names);
        const history = [];
        for (const p of nodes) {
            const body = `${String(p?.title || '')} ${String(p?.body || '')}`;
            if (allNames.some(n => body.includes(n))) {
                history.push(`[${String(p?.title || '前情').trim()}] ${cut(p?.body, Math.max(60, cap(maxTraitChars) * 2))}`);
                if (history.length >= cap(maxHistory)) break;
            }
        }
        if (history.length) lines.push(`前史（原著最早的相关剧情）：\n${history.join('\n')}`);
    }

    if (!lines.length) return '';
    return `${lines.join('\n')}\n（以上人物与前史来自原著分析，仅作把握性格/称谓/关系进展的参考；不要编造新设定，不要改变原文情节走向）`;
}
...
export function canEnrichChapter(ch) {
    ...
    if (!ch.judge) return false;
    // 跨章归位防御（最先判断）：缺口已归上一章章末（gapInPrev / seamToPrev）的章，
    // 即使模型矛盾输出（scene_type=explicit_sex + gap_in_prev 并存）也不让本章重复加料同一场景
    if (ch.judge.gapInPrev || ch.judge.seamToPrev) return false;
    // scene_type 驱动：只有 explicit_sex 才可加料
    if (ch.judge.sceneType === 'explicit_sex') return true;
    if (ch.judge.sceneType && ch.judge.sceneType !== 'explicit_sex') return false;
    // 旧格式兼容：无 scene_type 时按 result 判定
    return ['yes', 'doubt'].includes(ch.judge.result);
}
