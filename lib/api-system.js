...
export function createProfileApiClient({
    ...
    async function callProfileApi(messages, { responseLength = null, signal = null, onDelta = null } = {}) {
        ...
        if (profile.useTavernPreset !== true) {
            const url = String(profile.url || '').trim();
            const model = String(profile.model || '').trim();
            if (!url || !model) throw new Error('请先在 API 设置中填写接口地址与模型');
        } else {
            ...
            if (missing.length) {
                ...
                throw new Error(`酒馆运行时配置缺少：${missing.join('、')}。请在酒馆「连接」设置中确认当前源（${source}）的${missing.join('、')}已配置（Custom 源的地址在「自定义 API 地址」栏）；或关闭「使用酒馆主预设」改用独立配置`);
            }
        }
        const useStream = profile.stream === true;
        ...
        return runWithSemaphore(semaphore, async () => {
            const serviceSettings = profile.useTavernPreset === true
