 ...
 * @param {object} opts
 *   - skipNames: string[] 按条目名称（精确/包含）跳过的预设条目
 *   - dropAssistant: boolean 丢弃 role=assistant 的预设条目（对话开场/思维链引导类预填，
 *     在加料这类后台任务里会诱导模型输出打招呼/开场白/闲聊，建议丢弃）
 * @returns {Promise<Array<{role:string, content:string}>>}
 */
async function niBuildTavernPresetPromptMessages({ skipNames = [], dropAssistant = false } = {}) {
    const skipList = (Array.isArray(skipNames) ? skipNames : []).map(s => String(s || '').trim()).filter(Boolean);
    return niWithTavernMacroSandbox(async (macroState) => {
        const presetEntries = niGetTavernPresetPromptEntries('quiet');
        const result = [];
        for (const prompt of presetEntries) {
            const name = String(prompt?.name || '').trim();
            if (dropAssistant && String(prompt?.role || '').toLowerCase() === 'assistant') continue;
            if (skipList.some(s => name === s || name.includes(s))) continue;
            const content = niSubstituteTavernPresetContent(prompt.content, '', macroState).trim();
            if (!content) continue;
            const role = niNormalizeTavernMessageRole(prompt.role);
            result.push({
        ...
        while (true) {
            ...
            if (signal) {
                ...
                const abortPromise = new Promise((_, rej) => {
                    const onAbort = () => rej(new Error('AbortError'));
                    signal.addEventListener('abort', onAbort, { once: true });
                    readPromise.then(() => signal.removeEventListener('abort', onAbort), () => signal.removeEventListener('abort', onAbort));
                });
                readResult = await Promise.race([readPromise, abortPromise]);
            } else {
...
async function callCleanApi(messages, { signal = null } = {}) {
    ...
    return runWithSemaphore(ApiSemaphore, async () => {
        ...
        if (!resp.ok) {
            // 先读 body 再 cleanup（cleanup 会解除超时/abort 保护，先清理后读可能挂死）
            let txt = '';
            try {
                txt = await resp.text();
            } catch (err) {
            ...
            cleanup();
            throw new Error(`API ${resp.status}: ${String(txt || '').slice(0, 200)}`);
        }
...
export function createProfileApiClient({
    ...
    async function callProfileApi(messages, { responseLength = null, signal = null, onDelta = null } = {}) {
        ...
        return runWithSemaphore(semaphore, async () => {
            ...
            if (!resp.ok) {
                // 先读 body 再 cleanup：cleanup 会清掉超时 timer 与外部 abort 监听，
                // 若先 cleanup 后读 body，代理只发响应头不发 body 时会无限挂起整个队列
                let txt = '';
                try {
                    txt = await resp.text();
                } catch (err) {
