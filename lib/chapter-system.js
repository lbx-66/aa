...
import { niParseZip } from './storage-system.js';
...
// JUDGED 边补充说明：详情弹窗「手动改判（是/否/存疑）」对未判定/失败/跳过章直接落结果，
// 属用户意图最高优先，与重判（先回 UNDETECTED 再走队列）并行存在。
const CHAPTER_TRANSITIONS = {
    [CHAPTER_STATUS.UNDETECTED]: [CHAPTER_STATUS.DETECTING, CHAPTER_STATUS.SKIPPED, CHAPTER_STATUS.JUDGED],
    ...
    [CHAPTER_STATUS.FAILED]: [CHAPTER_STATUS.DETECTING, CHAPTER_STATUS.ENRICHING, CHAPTER_STATUS.UNDETECTED, CHAPTER_STATUS.JUDGED],
    [CHAPTER_STATUS.SKIPPED]: [CHAPTER_STATUS.DETECTING, CHAPTER_STATUS.ENRICHING, CHAPTER_STATUS.UNDETECTED, CHAPTER_STATUS.JUDGED],
};
...
/** 字符偏移 → 行号（基于真实行起始偏移二分；修复 CRLF 源下 split 行号错位问题）。 */
function lineOfCharOffset(lineStarts, offset) {
    const starts = Array.isArray(lineStarts) ? lineStarts : [0];
    const pos = Math.max(0, Number(offset) || 0);
    let lo = 0;
    let hi = starts.length - 1;
    if (pos <= starts[0]) return 0;
    if (pos >= starts[hi]) return hi;
    while (lo < hi) {
        const mid = (lo + hi + 1) >> 1;
...
export function recognizeChapters(text, { patterns = DEFAULT_CHAPTER_PATTERNS, customPatterns = [], boundaries = null } = {}) {
    ...
    if (hits.length) {
        ...
        // 后备：按 EPUB spine 文件边界切章（行号用真实行起始偏移换算，兼容 CRLF）
        const starts = boundaries.map(b => lineOfCharOffset(lineStarts, b.offset));
 ...
 * 支持 STORE 与 DEFLATE 压缩的 ZIP（外部 EPUB 通常是 DEFLATE）。
 */
export async function niExtractEpubText(buf) {
    let files;
    try {
        files = await niParseZip(buf);
    } catch (error) {
    ...
    if (rootPath) {
        ...
        if (opf) {
            ...
            while ((refMatch = idrefRe.exec(xml))) {
                const rawHref = manifest[refMatch[1]];
                if (!rawHref) continue;
                // 去掉 URL 片段/查询并解码（href 偶尔带 #/?(fragment) 或 %20 等转义），避免文件查找失败
                let clean = String(rawHref).replace(/[#?].*$/, '');
                try { clean = decodeURIComponent(clean); } catch (_) { /* 保留原样 */ }
                order.push(dir + clean.replace(/\\/g, '/'));
            }
        }
    }
    if (!order.length) {
