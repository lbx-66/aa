...
export const JUDGE_SUPPLEMENT_RULES = `【判定补充规则（与上方模板同时生效；模板与本节冲突处，一律以本节为准）】

一、跨章承接与缺口归属（结合上方上下文片段判断本章性爱线索是否真的属于本章）：
1. 承接上章——本章的性爱相关线索全部只是承接上一章已发生的性爱（前章结尾片段显示性爱已发生或已用省略/隐喻留白交代，或前章判定为 explicit_sex），本章没有新的性爱推进（没有脱衣、触摸私密部位、性行为过程在本章开始）→ scene_type 不得选 explicit_sex，has_gap: false，evidence 注明「承接上一章场景，本章无独立性爱情节」。即使命中上方模板的「事后拼图/组合C/省略跳转/只有事后缺过程」等写法，只要指向上一章，也一律按承接处理。
2. 两章间省略——上一章结尾止于铺垫（独处/关门/灭烛/亲吻升温等，未写性爱也无省略留白），本章开头出现次日/事后/昨夜回指承接 → 性爱被省略在两章之间，本章不得判 explicit_sex，has_gap: false，并输出 "gap_in_prev_chapter": true（表示补全应落在上一章章末）。
...
5. 宁缺毋滥——无法确认是否承接（如孤立"次日清晨"且无事后痕迹、无昨夜回指、无前章承接上下文）→ 按 romance/neutral 处理，不要输出 gap_in_prev_chapter。
...
export const BATCH_JUDGE_SUPPLEMENT_RULES = `【判定补充规则（批量扫描版，与上方模板同时生效；冲突处以本节为准）】

一、跨章承接与缺口归属：
1. 承接上章——某章的性爱线索全部只是承接上一章已发生的性爱（材料中该章前章信息显示性爱已发生/已省略留白，或前章判定 explicit_sex），该章没有新的性爱推进 → 该章 scene_type 不得选 explicit_sex，has_gap: false，evidence 注明「承接上一章场景，本章无独立性爱情节」。即使命中模板的「事后拼图/组合C/省略跳转」等写法，只要指向上一章，一律按承接处理。
2. 两章间省略——该章上一章结尾止于铺垫（独处/关门/亲吻升温等），该章开头承接次日/事后/昨夜回指 → 该章不得判 explicit_sex，has_gap: false，并输出 "gap_in_prev_chapter": true（补全应落在上一章章末）。
3. 本章自有场景 → 正常判 explicit_sex + has_gap: true。
4. 本章既有承接又有自有新场景 → 判 explicit_sex + has_gap: true，enrich_hints 第一条注明「不要以回忆/闪回补写上一章已发生的场景」。
5. 宁缺毋滥——无法确认承接时按 romance/neutral 处理，不输出 gap_in_prev_chapter。
