...
## 判定标准（has_intimacy: true = 存在可补全的性爱情节缺口）

一、有缺口 → has_intimacy: true
- **条件**：性爱情节已按"综合判定原则"成立（满足组合A/B/C）。
- **置信度分档**：
...
- **注意**：即使性爱过程完整，只要性爱情节成立，一律 has_intimacy: true。

二、无缺口 → has_intimacy: false，confidence 0.65~0.99
- **唯一情况**：性爱情节不成立（未满足组合A/B/C），且未命中安全排除。
- 例如：仅共处一室、仅一个吻、仅脸红心跳、仅环境铺垫而无推进；或内容为战斗、普通亲密、亲属互动等。
- **不再有"场景已完整"作为无缺口的理由。**

三、模糊地带 → confidence 0.4~0.59（has_intimacy 可 true 可 false；插件会标记「存疑」）
- 仅适用于无法确定是否构成性爱情节的情形（如仅有高度隐喻且无任何其他类线索组合）。
...
## 判定标准（has_gap: true = 该章存在可补全的性爱情节缺口）
...
- **置信度分档**：
  ...
  - 若性爱描写较为简略（有简单动作但无详细前戏/过程/高潮），confidence 0.80~0.90。
  - 若性爱描写已经详细（包含前戏、插入、高潮、事后等），仍判为有缺口，但置信度可略低，设为 0.75~0.85（因为虽然过程已写，但用户仍希望进一步丰富）。
- **注意**：即使性爱过程完整，只要性爱情节成立，一律 has_gap: true。
