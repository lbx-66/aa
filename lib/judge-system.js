 ...
 */
export const DEFAULT_JUDGE_PROMPT = `你是小说亲密场景分析器。本任务只做一件事：分析给定章节的亲密/暧昧场景描写，判断是否存在「可补全的缺口」，并输出结构化 JSON。不要角色扮演，不要输出分析过程。
 ...
 */
export const BATCH_JUDGE_PROMPT = `你是小说批量场景扫描器。下面给出多章小说的场景扫描材料，每章包含：章节编号、场景窗口位置、命中的场景阶段词、窗口原文摘录。请逐章判断该章是否存在「可补全的亲密场景缺口」，并输出结构化 JSON。不要角色扮演，不要输出分析过程。
