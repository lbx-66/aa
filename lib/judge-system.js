...
// mutual=互慰(未插入，含手交/边缘抚慰) / unclear=无法确定
export const SCENE_SUBTYPE_LABELS = {
    intercourse: '性交/插入',
    oral: '口交',
    solo: '单人自慰',
    mutual: '互慰(未插入)',
...
export const SCENE_SUBTYPES = ['intercourse', 'oral', 'solo', 'mutual', 'unclear'];
...
export function sceneSubtypeLabel(sceneSubtype) {
    return SCENE_SUBTYPE_LABELS[sceneSubtype] || '';
}

/** 归一化 AI 返回的场景子类型：空值/未给 → undefined（未知）；非法值 → 'unclear'。 */
export function normalizeSceneSubtype(value) {
    if (value === undefined || value === null) return undefined;
    const v = String(value).trim().toLowerCase();
    if (!v) return undefined;
    return SCENE_SUBTYPES.includes(v) ? v : 'unclear';
}

/** 归一化 AI 返回的加料建议：接受字符串数组/单字符串，截断条数与单条长度。 */
export function normalizeEnrichHints(value, { cap = 3, maxLen = 80 } = {}) {
    const list = Array.isArray(value)
        ? value
        : (String(value || '').trim() ? String(value).split(/\n|；|;/).filter(Boolean) : []);
    return list
        .map(v => String(v || '').trim().replace(/\s+/g, ' '))
        .filter(Boolean)
        .slice(0, Math.max(1, Number(cap) || 3))
 ...
 * @param {object} opts { sceneConfig?, bookProfile?, chapterNumber?, autoNames?, prevScored? }
 *   prevScored：上一章关键词评分结果（用于跨章承接降级：上一章已判定的性爱，本章
 *   只有"次日/事后/昨夜回指"的非核心窗口时不再当作本章缺口）。
 ...
 *            bookProfile:string, modes:string[], mode:'keyword', carryover?:object}}
 */
export function scoreIntimacy(text, rules = {}, opts = {}) {
    ...
    // 跨章承接降级：上一章已发生/已判定性爱时，本章的"事后/次日承接"窗口不再作为缺口
    let carryover = null;
    if (opts.prevScored && typeof opts.prevScored === 'object') {
        const prevWindows = Array.isArray(opts.prevScored.scenes)
            ? opts.prevScored.scenes
            : (Array.isArray(opts.prevScored.windows) ? opts.prevScored.windows : []);
        const flag = flagCarryoverWindows(analysis.windows, {
            hit: opts.prevScored.hit === true,
            windows: prevWindows,
        });
        if (flag.suppressed > 0) {
            carryover = { suppressed: flag.suppressed, carryoverOnly: flag.carryoverOnly };
            analysis.actionableCount = analysis.windows.filter(w => w.completionMode).length;
            analysis.hit = analysis.actionableCount > 0 && !analysis.vetoed && analysis.vetoCount === 0;
        }
    }
...
// ------------------------------------------------------------
// 判定补充规则（自动追加在模板之后，与模板同时生效；冲突处以本节为准）。
// 独立于用户可编辑模板，保证跨章承接 / 加料建议 / 场景子类型等新语义
// 对旧模板与自定义模板同样生效，无需迁移存量模板。
// ------------------------------------------------------------

export const JUDGE_SUPPLEMENT_RULES = `【判定补充规则（与下方模板同时生效；模板与本规则冲突处，一律以本规则为准）】

【执行顺序（每次判定先按五步内部推理，不要输出过程）】
第 1 步 摘录：只在本章正文中找出与性爱相关的句子/段落（脱衣、私密接触、行为过程、省略留白、隐喻、事后状态、次日承接），记下所在段号。
第 2 步 定性：本章是否发生或明确暗示发生了性爱行为？注意：单人自慰/自我抚慰也属性爱行为，不需要第二人；仅亲吻拥抱、脸红心跳、暧昧拉扯不算。
第 3 步 归属：当晚进程（喝酒回房/独处/脱衣/熄灯/省略留白/次日醒来等）是否都在本章内？都在本章 → 归属本章（见下方情形 3）；本章自开头即承接上一章且全章找不到当晚任何进程 → 才考虑承接或 gap_in_prev_chapter（见情形 1/2）。
第 4 步 缺口：性爱过程被省略/中断/一句带过 → has_gap: true；性爱情节不成立 → has_gap: false。
第 5 步 一致性自检：结论为 explicit_sex 时，evidence 必须能引用本章原文片段（含段号）；若你写出的证据已经证明性爱发生而结论是否/纯爱 → 结论必须改为 explicit_sex；若找不到任何可引用的本章原文 → 不得判 explicit_sex。

【引用纪律】
evidence 一律写成「第 N 段：原文短引」格式；引用只允许来自本章正文；上方【上下文参考】里的前章判定/前章结尾片段/后章开头片段与前后章摘要只用于第 3 步归属判断，绝不允许作为本章性爱证据出现在 evidence 里。

一、跨章承接与缺口归属（结合上方上下文片段判断本章性爱线索是否真的属于本章）：

0. 上下文边界（先读这条）：上方【上下文参考】里的前章判定/前章结尾片段/后章开头片段，以及前后章摘要，**只用于判断「本章开头是否承接上一章、本章结尾是否被下一章承接」**；前后章里发生的事件一律不是本章内容，evidence 只能引用本章正文片段，绝不可把相邻章节的句子当作本章证据引用。

1. 承接上章——前提是**本章自身不含当晚的任何性爱铺垫或进程**（如本章内写到该夜的饮酒/回房/独处/脱衣/熄灯/同床/省略留白/隐喻等）。满足前提且本章的性爱相关线索全部只是承接上一章已发生的性爱（前章结尾片段显示性爱已发生或已用省略/隐喻留白交代，或前章判定为 explicit_sex），本章没有新的性爱推进 → scene_type 不得选 explicit_sex，has_gap: false，evidence 注明「承接上一章场景，本章无独立性爱情节」。若本章内存在当晚进程文字（哪怕事后状态与它同章出现），一律按情形 3 处理，不得按承接判否。

2. 两章间省略——**章内闭环必须优先排除**：若该夜的「睡前铺垫 + 省略/隐喻留白 + 次日醒来/事后状态」全部出现在本章正文内（例如本章从晚上/醉酒/回房写起，章内出现"次日清晨/醒来"），这是本章内的省略留白闭环，必须判 scene_type 为 explicit_sex、has_gap: true（补全当晚场景，补在本章省略处），**禁止输出 gap_in_prev_chapter**。只有当下述全部成立时才允许 gap_in_prev_chapter: true：上一章结尾止于铺垫（未写性爱也无省略留白）＋ 本章正文从开头起就是次日/事后/昨夜回指承接 ＋ 整个本章内找不到该夜任何铺垫或性爱进程文字（本章没有"入夜/回房/脱衣/熄灯"等当晚进程）。

3. 本章自有场景——性爱推进确实发生在本章（含本章内的省略留白、环境转喻、古典隐喻一步到位）→ 正常判 explicit_sex + has_gap: true，缺口归属本章。
4. 本章既承接上一章又有自有新场景 → 按新场景正常判 explicit_sex + has_gap: true；enrich_hints 第 1 条必须注明「上一章已发生的场景不要在本章以回忆/闪回方式补写」。
5. 宁缺毋滥——无法确认是否承接（如孤立"次日清晨"且无事后痕迹、无昨夜回指、无前章承接上下文）→ 按 romance/neutral 处理，不要输出 gap_in_prev_chapter；无法确认归属时倾向归本章（本章内能找到当晚进程即本章）。
6. 单人自慰/自我抚慰场景（scene_subtype: "solo"）——**本插件的加料目标只包含「女性角色」的单人自慰/自我抚慰**：执行者必须是明确的女性角色（她/女主/女角色名或上下文唯一指代）。**男性角色单人自慰（手淫/打飞机/自己解决等）不属于加料目标：一律不得判 explicit_sex**（按其实际语境判 neutral/romance/romantic_tension），也不要输出 scene_subtype: "solo"；无法确定执行者性别时宁缺毋滥不判。确认为女性自慰后按下述门槛：
 - 判 explicit_sex + scene_subtype "solo" 的门槛（满足其一即可）：
   (a) 出现明确的女性自我性行为描写：自慰/手淫/自我抚慰/自我纾解，或抚摸/揉弄/揉搓/玩弄自己身体，或夹紧双腿磨蹭、指尖在自己身上游走等（含拟物、委婉写法）；
   (b) 女性角色处于私密/独处空间（房内/床上/浴室/锁上门）并伴随明确性唤起状态（情欲高涨/想要/难耐/空虚/燥热），随后出现自我抚慰动作，或性唤起被叙事中断留白（省略号/分节/跳转次日）——两者缺一不可；
   (c) (b) 的性唤起若有明确来源铺垫（本章或紧邻章出现勾起欲望的对象/照片/回忆/自拍等），且随后是奔向独处的行动，可放宽为"性唤起积累 + 独自离开去私密空间"即判（宁缺毋滥下限）。
 - 判定后按描写简略程度给 has_gap 与置信度：只写了"自慰/回家自慰"性质的一句带过 → has_gap: true（补全独处自慰过程）；已展开过程但无高潮/结尾 → has_gap: true；已完整 → 按定制语义仍 has_gap: true。
 - 宁缺毋滥边界：仅"性唤起 + 回房/回家……不能再等了"这类收尾暗示、**没有任何**自我抚慰动作、也没有中断留白的，不判 explicit_sex（按 romantic_tension/romance），evidence 说明「结尾为性唤起与独处暗示，未明写自慰行为，未判；如确为自慰收尾请人工标记后加料」；不得因为猜测下一章会出现自慰内容而提前判本章。

二、输出 JSON 的补充字段（可选，仅在相应情形输出）：
- "enrich_hints"：字符串数组，仅当 has_gap: true（或 gap_in_prev_chapter: true 时为上一章章末补全建议）时给出 1~3 条，每条 ≤80 字；内容 = 缺口位置（第几段附近 + 一句原文短引）+ 建议补全的动作顺序/要点 + 类型注意（如"保持单人自慰，不引入第二人""未插入的边缘推进"）。若该章另有承接上一章的部分，在第一条注明不要写回忆/闪回。
- "scene_subtype"："intercourse"|"oral"|"solo"|"mutual"|"unclear"。仅当 scene_type 为 explicit_sex 且能确定行为类型时给出：intercourse=插入性交；oral=口交；solo=单人自慰/自我抚慰；mutual=互慰/手交/未插入的边缘抚慰；unclear=仅有省略隐喻无法确定。宁缺毋滥：无法确定就不给或给 "unclear"。
- "gap_in_prev_chapter"：true（仅上面情形 2）。

三、输出整体要求（覆盖模板中与此冲突的表述）：只输出一个 JSON 对象；chapters/章节字段名与模板一致；除 JSON 外不输出任何文字。

【输出示例】
正确：{"index": 12, "scene_type": "explicit_sex", "has_gap": true, "confidence": 0.9, "evidence": "第 5 段：他解开她的衣襟；第 7 段：此处省略——当晚铺垫与省略均在本章，缺口在本章省略处", "enrich_hints": ["在第 7 段省略处展开当晚场景"]}
错误：{"index": 13, "scene_type": "neutral", "has_gap": false, "confidence": 0.95, "evidence": "第 2 段引用了上一章摘要中的句子"}（错误原因：引用越界 + 证据与结论矛盾——evidence 已指向性爱却判 neutral）。`;

export const BATCH_JUDGE_SUPPLEMENT_RULES = `【判定补充规则（批量扫描版，与下方模板同时生效；冲突处以本规则为准）】

【批量执行纪律（先读这条）】
- 逐章独立判定：每章的结论只依据该章自己的材料（场景窗口分析、原文摘录、本章开头/结尾片段、本章情报）；【上下文链】只解释承接与归属，绝不作为该章性爱证据；
- 每章判定前做三步自检（不输出过程）：定性（该章材料里是否有性爱行为/明示暗示？）→ 归属（当晚进程是否在该章材料内？）→ 一致性（判 explicit_sex 必须能在该章材料中引用到原文，引不到不得判「是」）；
- 材料标注「（本材料未发现场景窗口）」的章：默认 scene_type "neutral"、has_gap: false，除非能在本章开头/结尾片段中找到可引用的性爱证据；
- chapters 必须覆盖材料中的每一章编号，不得遗漏、不得合并、不得编造编号；
- 一次输出完整 JSON，不要在末尾追加任何说明文字。

【引用纪律】
evidence 一律写成「第 N 段/窗口 X：原文短引」格式；引用只允许来自该章自己的窗口摘录或开头/结尾片段；前后章摘要/前章尾段只用于归属判断，绝不允许作为该章性爱证据出现在 evidence 里。

一、跨章承接与缺口归属：

0. 上下文边界（先读这条）：材料中每章的【上下文链】（前章摘要/前章判定/前章尾段/后章摘要）**只用于判断「本章开头是否承接上一章、本章结尾是否被下一章承接」**；相邻章节里发生的事件一律不是本章内容，evidence 只能引用本章材料里的窗口与摘录，绝不可把前后章句子当作本章证据引用。

1. 承接上章——前提是**该章自身不含当晚的任何性爱铺垫或进程**（如该章内写到当晚的饮酒/回房/独处/脱衣/熄灯/同床/省略留白/隐喻等）。满足前提且该章性爱线索全部只是承接上一章已发生的性爱（材料中该章前章信息显示性爱已发生/已省略留白，或前章判定 explicit_sex），该章没有新的性爱推进 → 该章 scene_type 不得选 explicit_sex，has_gap: false，evidence 注明「承接上一章场景，本章无独立性爱情节」。若该章内存在当晚进程文字，一律按情形 3 处理。

2. 两章间省略——**章内闭环必须优先排除**：若当晚的「睡前铺垫 + 省略/隐喻留白 + 次日醒来/事后状态」全部出现在该章正文内（该章从晚上/醉酒/回房写起、章内有"次日清晨/醒来"），这是该章内的省略留白闭环，必须判 explicit_sex + has_gap: true（缺口在本章省略处），**禁止输出 gap_in_prev_chapter**。只有下述全部成立才允许 gap_in_prev_chapter: true：上一章结尾止于铺垫（未写性爱也无省略留白）＋ 该章正文从开头就是次日/事后/昨夜回指承接 ＋ 整个该章找不到当晚任何铺垫或性爱进程文字。
3. 本章自有场景 → 正常判 explicit_sex + has_gap: true。
4. 本章既有承接又有自有新场景 → 判 explicit_sex + has_gap: true，enrich_hints 第一条注明「不要以回忆/闪回补写上一章已发生的场景」。
5. 宁缺毋滥——无法确认承接时按 romance/neutral 处理，不输出 gap_in_prev_chapter；无法确认归属时倾向归本章（本章能找到当晚进程即本章）。
6. 单人自慰/自我抚慰场景（scene_subtype: "solo"）——**只包含「女性角色」的单人自慰/自我抚慰**：执行者必须是明确的女性角色（她/女主/女角色名/上下文唯一指代）。**男性角色单人自慰（手淫/打飞机/自己解决等）一律不得判 explicit_sex**（按其语境判 neutral/romance/romantic_tension），也不要输出 scene_subtype: "solo"；无法确定执行者性别时宁缺毋滥不判。确认为女性自慰后按下述门槛（满足其一）：
   (a) 材料中出现明确的女性自我性行为描写：自慰/手淫/自我抚慰，或抚摸/揉弄/揉搓/玩弄自己、夹紧双腿磨蹭、指尖在身上游走等（含委婉/拟物写法）；
   (b) 女性角色独处私密空间（房内/床上/浴室/锁门）＋ 明确性唤起状态 ＋ 随后有自我抚慰动作或叙事中断留白（省略/分节/跳次日），缺一不可；
   (c) (b) 的性唤起有明确来源铺垫（对象/照片/回忆等）且随后是奔向独处的行动，可放宽为"性唤起积累＋独自离开去私密空间"即判（宁缺毋滥下限）。
   一句带过 → has_gap: true（补全自慰过程）；仅"性唤起＋回房/回家……不能再等了"收尾暗示、无动作也无留白的 → 不判 explicit_sex，evidence 说明并建议人工标记，不得因猜测下一章有自慰内容而提前判本章。
7. 无窗口章——材料标注「（本材料未发现场景窗口）」或只给了本章开头片段的章：默认 scene_type 为 "neutral"/"romantic_tension"、has_gap: false。只有当本章材料/开头片段里存在明确的性爱证据（脱衣/上床/私密接触/性爱隐喻/明确事后状态等）并且你能在 evidence 中引用到具体原文时，才允许判 explicit_sex；绝不允许在没有任何本章原文佐证时仅凭相邻章节信息判 explicit_sex 或 has_gap: true。

二、输出 JSON 的补充字段（可选，仅相应情形输出）：
- "enrich_hints"：字符串数组，仅当 has_gap: true 时给出，每章至多 2 条、每条 ≤60 字：缺口位置（第几段附近 + 一句原文短引）+ 补全要点 + 类型注意（"保持单人自慰不引入第二人""未插入的边缘推进"等）。
- "scene_subtype"："intercourse"|"oral"|"solo"|"mutual"|"unclear"，仅 explicit_sex 且能确定行为类型时给出（intercourse=插入性交；oral=口交；solo=单人自慰；mutual=互慰/手交/未插入边缘；unclear=仅省略隐喻无法确定）。
- "gap_in_prev_chapter"：true（仅情形 2）。

三、输出整体要求（覆盖模板中冲突表述）：只输出一个 JSON 对象，chapters 覆盖全部材料章节，index 与材料一致；除 JSON 外不输出任何文字。`;
...
/** 构建 AI 判定的 messages（单条 system，模板已含正文 + 自动追加判定补充规则）。 */
export function buildJudgeMessages(template, { chapterContent = '', rulesSummary = '', context = '' } = {}) {
    ...
    // 补充规则置前（消息开头的短规则遵循度最高），模板中的六类线索详解随后，
    // 末尾再收一句「宁缺毋滥 + 只输出 JSON」强化输出纪律
    return [{ role: 'system', content: `${JUDGE_SUPPLEMENT_RULES}\n\n${filled}\n\n【最终输出要求（重申）】宁缺毋滥：无法确认就不判「是」；只输出一个 JSON 对象，不要输出任何其他文字。` }];
}
...
export function buildBatchChaptersText(chapters, { sceneConfig = {}, bookProfile = 'normal', autoNames = [], maxCharsPerChapter = 1200 } = {}) {
    ...
    for (const ch of (Array.isArray(chapters) ? chapters : [])) {
        ...
        // 上下文链：前章摘要/前章判定/前章尾段 + 后章摘要（由调用方附加，
        // 用于判断本章性爱线索是承接上一章、两章间省略还是本章自有）
        const ctxParts = [];
        if (ch?.prevSummary) ctxParts.push(`前章摘要：${ch.prevSummary}`);
        if (ch?.prevJudge) ctxParts.push(`前章判定：${ch.prevJudge}`);
        if (ch?.prevTail) ctxParts.push(`前章结尾片段：${ch.prevTail}`);
        if (ch?.nextSummary) ctxParts.push(`后章摘要：${ch.nextSummary}`);
        if (ctxParts.length) lines.push(`【上下文链】\n${ctxParts.join('\n')}`);
        // 本章开头片段（由调用方按需附加）：窗口摘录只覆盖省略/事后窗口附近，
        // 无法判断"当晚铺垫是否在本章前部"；对无窗口章与省略/事后窗口章附本章开头，
        // 让 AI 能分辨「本章内闭环省略」与「承接上一章/两章间省略」
        const headSnip = String(ch?.headSnippet || '').trim();
        if (headSnip) lines.push(`本章开头片段：${headSnip}`);
        // 本章结尾片段（可选）：收尾类信号（如性唤起后奔向独处的暗示收尾）在章尾，
        // 无窗口章的素材若只有章首会完全错过
        const tailSnip = String(ch?.tailSnippet || '').trim();
        if (tailSnip) lines.push(`本章结尾片段：${tailSnip}`);
...
/** 构建批量扫描 messages（单条 system，模板已含材料与要求 + 自动追加批量判定补充规则）。 */
export function buildBatchJudgeMessages(template, { chaptersText = '', rulesSummary = '' } = {}) {
    const filled = String(template || '')
        ...
        .replaceAll('{rules}', String(rulesSummary || ''));
    // 补充规则置前；批量材料较长，最后重申覆盖与输出纪律
    return [{
        role: 'system',
        content: `${BATCH_JUDGE_SUPPLEMENT_RULES}\n\n${filled}\n\n【最终输出要求（重申）】chapters 必须覆盖材料中的每一章（含「（本材料未发现场景窗口）」的章，输出 scene_type: "neutral"、has_gap: false）；宁缺毋滥：无法引用本章原文确认的不判「是」；只输出一个 JSON 对象，不要输出任何其他文字。`,
    }];
}
...
/**
 * 解析批量场景扫描响应。
 * 支持新格式 {chapters: [{index, scene_type, has_gap, confidence, evidence,
 *   enrich_hints?, scene_subtype?, gap_in_prev_chapter?}]} 和旧格式。
 * @returns {Map<number, {result:'yes'|'no', confidence:number, evidence:string, sceneType?:string,
 *   sceneSubtype?:string, enrichHints?:string[], gapInPrev?:boolean}>} index → 判定
...
export function parseBatchJudgeResponse(raw) {
    ...
    for (const item of list) {
        ...
        byIndex.set(index, {
            ...
            sceneSubtype: normalizeSceneSubtype(item.scene_subtype),
            enrichHints: normalizeEnrichHints(item.enrich_hints || item.enrich_hint || item.adding_suggestions, { cap: 2, maxLen: 60 }),
            gapInPrev: item.gap_in_prev_chapter === true || item.gap_in_prev === true,
        });
    }
    return byIndex;
}
...
/**
 * 解析 AI 判定响应。
 * 支持新格式 {scene_type, has_gap, confidence, evidence, enrich_hints?, scene_subtype?,
 *   gap_in_prev_chapter?} 和旧格式 {has_intimacy, confidence, evidence}。
 * @returns {{result:'yes'|'no', confidence:number, evidence:string, sceneType?:string,
 *   sceneSubtype?:string, enrichHints?:string[], gapInPrev?:boolean}}
 * @throws 响应不是有效 JSON 或缺少关键字段
 */
export function parseJudgeResponse(raw) {
    const obj = extractJsonObject(raw);
    const extra = {
        sceneSubtype: normalizeSceneSubtype(obj.scene_subtype),
        enrichHints: normalizeEnrichHints(obj.enrich_hints || obj.enrich_hint || obj.adding_suggestions, { cap: 3, maxLen: 80 }),
        gapInPrev: obj.gap_in_prev_chapter === true || obj.gap_in_prev === true,
    };
    // 新格式：scene_type 驱动
    if (obj.scene_type) {
        ...
        return { result, confidence, evidence, sceneType, ...extra };
    }
    ...
    const confidence = Math.max(0, Math.min(1, Number(obj.confidence) || 0));
    const evidence = String(obj.evidence || obj.reason || obj.summary || '').trim();
    return { result, confidence, evidence, ...extra };
}
 ...
 *  sceneType：安全否决不设类型；「是」→ explicit_sex；有窗口但无缺口（存疑）不设；
 *  无窗口「否」→ neutral。sceneSubtype 仅 explicit_sex 时按窗口活动类型推断。 */
export function keywordJudgeToStore(scored) {
    const scenes = Array.isArray(scored.scenes) ? scored.scenes : [];
    const isYes = scored.vetoed ? false : scored.hit === true;
    const sceneType = scored.vetoed ? undefined : (isYes ? 'explicit_sex' : ((scored.hitCount || 0) > 0 ? undefined : 'neutral'));
    const sceneSubtype = isYes ? chapterSceneSubtype(scenes) : undefined;
    let evidence = scored.evidence || (scored.hitCount > 0 ? `共 ${scored.hitCount} 处场景窗口` : '未发现场景窗口');
    if (scored.carryover?.suppressed) {
        const suffix = scored.carryover.carryoverOnly
            ? `（跨章承接：${scored.carryover.suppressed} 处窗口为上一章性爱场景的次日/事后承接描写，已按承接处理，本章不另加料）`
            : `（跨章承接：${scored.carryover.suppressed} 处窗口为上一章场景的承接描写，已从本章缺口中排除）`;
        evidence = `${evidence}\n${suffix}`;
    }
    return {
        ...
        sceneType,
        sceneSubtype,
        carryover: scored.carryover || null,
