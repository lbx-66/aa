 ...
 * has_intimacy 在此语义下 = 存在可补全的性爱情节缺口（true → 判「是」可加料）。
 */
export const DEFAULT_JUDGE_PROMPT = `你是小说性爱情节分析器。本任务只做一件事：分析给定章节是否存在「可补全的性爱情节缺口」，并输出结构化 JSON。不要角色扮演，不要输出分析过程。
 ...
 * 普通亲密/浪漫/纯爱互动不算缺口，防止误判送去加料。配合 parseBatchJudgeResponse 使用。
 */
export const BATCH_JUDGE_PROMPT = `你是小说批量性爱情节扫描器。下面给出多章小说的场景扫描材料，每章包含：章节编号、场景窗口位置、命中的场景阶段词、窗口原文摘录。请逐章判断该章是否存在「可补全的性爱情节缺口」，并输出结构化 JSON。不要角色扮演，不要输出分析过程。
...
/** 从 AI 响应中提取 JSON 对象（剥代码围栏 + 裸 JSON 抢救）；失败抛错。 */
