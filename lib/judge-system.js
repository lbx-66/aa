...
6. 单人自慰/自我抚慰场景（scene_subtype: "solo"）——这类情节不涉及第二人，按下列通用门槛判定，不要因为"不是双人性爱"就不判：
 - 判 explicit_sex + scene_subtype "solo" 的门槛（满足其一即可）：
   (a) 出现明确的自我性行为描写：自慰/手淫/自我抚慰/自我纾解，或抚摸/揉弄/揉搓/玩弄自己身体，或夹紧双腿磨蹭、指尖在自己身上游走等（含拟物、委婉写法）；
   (b) 单人处于私密/独处空间（房内/床上/浴室/锁上门）并伴随明确性唤起状态（情欲高涨/想要/难耐/空虚/燥热），随后出现自我抚慰动作，或性唤起被叙事中断留白（省略号/分节/跳转次日）——两者缺一不可；
...
6. 单人自慰/自我抚慰场景（scene_subtype: "solo"）——不涉及第二人的性情节也属于 explicit_sex，门槛（满足其一）：
   (a) 材料中出现明确自我性行为描写：自慰/手淫/自我抚慰，或抚摸/揉弄/揉搓/玩弄自己、夹紧双腿磨蹭、指尖在身上游走等（含委婉/拟物写法）；
   (b) 独处私密空间（房内/床上/浴室/锁门）＋ 明确性唤起状态 ＋ 随后有自我抚慰动作或叙事中断留白（省略/分节/跳次日），缺一不可；
...
6. 无窗口章——材料标注「（本材料未发现场景窗口）」或只给了本章开头片段的章：默认 scene_type 为 "neutral"/"romantic_tension"、has_gap: false。只有当本章材料/开头片段里存在明确的性爱证据（脱衣/上床/私密接触/性爱隐喻/明确事后状态等）并且你能在 evidence 中引用到具体原文时，才允许判 explicit_sex；绝不允许在没有任何本章原文佐证时仅凭相邻章节信息判 explicit_sex 或 has_gap: true。
