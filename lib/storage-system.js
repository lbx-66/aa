...
export function niLegacyServerFileNames(key) {
    const id = niServerFileId(key).replace(/[\\/:*?"<>|]/g, '_');
    const safe = String(key || '').replace(/[\\/:*?"<>|]/g, '_');
    return [
        `novel_injector_${id}.json`,
        `novel_injector_${safe}.json`,
        `ni_data_${safe}.json`,
    ];
}
...
/**
 * 异步 ZIP 解析（STORE 直拷 + DEFLATE 解压）：
 * 外部 EPUB 几乎都是 DEFLATE 压缩，_parseZip 只能读 STORE（本插件自产）。
 * method 8 条目用 DecompressionStream('deflate-raw') 解压（Chrome/Edge/现代 Safari/安卓 WebView 支持）；
 * 不支持的压缩方式（如 bzip2/lzma）跳过该条目并告警。
 * @returns {Promise<Object<string, Uint8Array>>}
 */
export async function niParseZip(buffer) {
    const view = new DataView(buffer);
    const u8 = new Uint8Array(buffer);
    const files = {};
    const inflateJobs = [];
    ...
    while (i < u8.length - 4) {
        ...
        if (method === 0) {
            ...
            inflateJobs.push({ name, comp });
        } else {
            console.warn('[NI] ZIP 条目使用了不支持的压缩方式（method', method + '），已跳过:', name);
        }
        i = dataStart + compSize;
    }
    for (const job of inflateJobs) {
        try {
            ...
            const stream = new Blob([job.comp]).stream().pipeThrough(ds);
            const ab = await new Response(stream).arrayBuffer();
            files[job.name] = new Uint8Array(ab);
        } catch (e) {
            console.warn('[NI] ZIP 条目解压失败，跳过:', job.name, e?.message || e);
        }
    }
    return files;
}
...
export function createStorageController(deps = {}) {
    ...
    async function niServerLoadJsonByNames(names) {
        const tasks = names.map(name => fetch(`/user/files/${name}`, {
            headers: getRequestHeaders(),
            cache: 'no-cache',
        }).then(res => {
            if (res.status === 404) return null;
            if (!res.ok) throw new Error(`服务端读取失败: ${res.status}`);
            return res.json().then(payload => ({ name, payload }));
        }));
        try {
